...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # initialize embedding to hidden layer weight and bias 
        embedd_w = nn.Parameter(torch.empty(n_features * self.embed_size, hidden_size))
        embedd_b = nn.Parameter(torch.empty(hidden_size))
        self.embed_to_hidden_weight = nn.init.xavier_uniform_( embedd_w)
        self.embed_to_hidden_bias = nn.init.uniform_( embedd_b)
        #hidden layer 
        logits_w = nn.Parameter(torch.empty(hidden_size, n_classes))
        logits_b = nn.Parameter(torch.empty(n_classes)) 
        self.hidden_to_logits_weight = nn.init.xavier_uniform_(logits_w)
        self.hidden_to_logits_bias = nn.init.uniform_(logits_b)
